...
**에이전트가 아예 호출할 수 없고**, 사용자가 슬래시 명령(`/prototype …`)을 직접 입력해야만
켜집니다. 5단계 검증에서 목록에 안 보인다고 설치 실패로 보고하지 마세요.
**파일 개수로 확인하세요.**

사용자에게 안내할 때 "`prototype` 스킬 써달라고 말하세요" 라고 하지 마세요 — 그렇게 하면
에이전트가 대신 호출하려다 거부당합니다. **`/prototype` 처럼 직접 입력하라고** 안내하세요.
